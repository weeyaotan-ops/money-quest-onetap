...
global.fetch=async function binanceRelayFetch(input,init={}){
  ...
  if(method!=='GET'||u.host!==HOST)return originalFetch(input,init);
  ...
  try{
    const rr=await originalFetch(RELAY_URL,{
      ...
      body:JSON.stringify({method:'GET',url:u.toString(),apiKey}),
      signal:AbortSignal.timeout(12000)
    });
    const j=await rr.json();
    if(!rr.ok||!j?.ok)throw Error('RELAY_HTTP_'+rr.status+'_'+String(j?.error||'unknown'));
    console.log('BINANCE_PRIVATE_EGRESS_GET',JSON.stringify({path:u.pathname,status:Number(j.status||0),usedWeight1m:j?.headers?.['x-mbx-used-weight-1m']||null}));
...
console.log('BINANCE_PRIVATE_EGRESS_READY',JSON.stringify({enabled:Boolean(RELAY_URL&&TOKEN),mode:'GET_ONLY',host:HOST}));
