...
// It only reduces repeated GET traffic for metadata/fee endpoints and backs off after 418/-1003.
...
const stats={network:0,cacheHits:0,syntheticCommission:0,bracketFallbacks:0,rateLimited:0,localCooldownBlocks:0,lastRateLimit:null};
let cooldownUntil=0;
function cloneResponse(body,status=200,headers={'content-type':'application/json'}){return new Response(body,{status,headers})}
function cooldownResponse(){stats.localCooldownBlocks++;return cloneResponse(JSON.stringify({code:-1003,msg:`Local Binance GET cooldown active until ${cooldownUntil}`}),418)}
...
function noteLimit(body){stats.rateLimited++;stats.lastRateLimit=new Date().toISOString();cooldownUntil=Math.max(cooldownUntil,banUntil(body))}
async function fetchAndCache(url,init,key,ttl,fallback){
  ...
  if(Date.now()<cooldownUntil)return fallback?fallback('COOLDOWN'):cooldownResponse();
  const p=(async()=>{
...
global.fetch=async function rateGuardFetch(input,init={}){
  ...
  if(r.status===418){const body=await r.text();noteLimit(body);return cloneResponse(body,r.status,{'content-type':r.headers.get('content-type')||'application/json'})}
  return r;
};
...
console.log('BINANCE_RATE_GUARD_READY',JSON.stringify({host:restHost,commissionMode:'CONSERVATIVE_FALLBACK',fallbackTakerBps:FALLBACK_TAKER*10000,metaCacheMs:META_TTL,bracketCacheMs:BRACKET_TTL,bracketFallbackLeverage:BRACKET_FALLBACK,writeRequestsUntouched:true,getCooldown:true}));
