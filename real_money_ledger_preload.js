...
const LOOKBACK_MS=Math.max(3600000,Number(process.env.REAL_MONEY_LOOKBACK_MS||7*24*3600000));
const UNKNOWN_RETRY_MS=Math.max(15*60*1000,Number(process.env.REAL_MONEY_UNKNOWN_RETRY_MS||6*60*60*1000));
const trades=new Map();let reconciling=false,lastReconcile=null,cooldownUntil=0,requestCount=0,lastError=null;
const seed=[
['hunter_8579afc83519319a4274a219','FFUSDT','BUY',281,.1417,1,1.390512764000002,1.7022579218841285,'2026-09-15T07:31:36.600Z'],['hunter_9384d52027fe8db881a467b3','USELESSUSDT','BUY',329,.19933118541033434,2,1.390870530000007,1.640594478624826,'2026-09-15T07:31:42.465Z'],['hunter_2e0c48b532a0298878577cd2','牛来USDT','BUY',838,.10975797136038187,4,1.391755428000003,1.5815333037091588,'2026-09-15T07:32:28.765Z'],['hunter_0ff1b560c24ce326b802b934','VTHOUSDT','BUY',59385,.0007176,3,1.3924618554000003,1.6937040366700158,'2026-09-15T07:33:24.848Z'],['hunter_dcf0c54baf1a66ba2d448c22','SKRUSDT','SELL',3428,.01735140840140023,5,1.411979404,1.6321719678568336,'2026-09-15T07:33:30.765Z'],['hunter_73cebe8bd8f3ba84645cf3f8','ACEUSDT','BUY',376.26,.15899,8,1.3919189360399944,1.6556263607105501,'2026-09-15T07:34:21.783Z'],['hunter_6b620175d8b214a7c65e221f','LITUSDT','SELL',20,4.3776,20,1.0716420000000046,1.5266120588778602,'2026-09-15T07:37:57.013Z'],['hunter_b187ac9e6fbdb12d93d49022','AVAAIUSDT','SELL',6778,.010176,13,1.411979404,1.6321719678568336,'2026-09-15T07:58:55.221Z'],['hunter_a29e5080c8b4bc4b79727c51','BULLAUSDT','SELL',1066,.07587756660412759,7,1.3971190011999977,1.6049136201526912,'2026-09-15T08:03:06.131Z']];
function save(){try{fs.mkdirSync(pathMod.dirname(STATE_FILE),{recursive:true});const tmp=STATE_FILE+'.tmp';fs.writeFileSync(tmp,JSON.stringify({version:2,savedAt:new Date().toISOString(),lastReconcile,trades:[...trades.values()]}));fs.renameSync(tmp,STATE_FILE);return true}catch(e){lastError=String(e?.message||e);originalLog('REAL_MONEY_LEDGER_PERSIST_ERR',lastError);return false}}
function load(){try{if(!fs.existsSync(STATE_FILE))return false;const j=JSON.parse(fs.readFileSync(STATE_FILE,'utf8'));for(const t of Array.isArray(j?.trades)?j.trades:[])if(t?.id)trades.set(String(t.id),t);lastReconcile=j?.lastReconcile||null;originalLog('REAL_MONEY_LEDGER_PERSIST_RESTORED',JSON.stringify({file:STATE_FILE,trades:trades.size,closed:[...trades.values()].filter(t=>t.status==='CLOSED').length}));return trades.size>0}catch(e){lastError=String(e?.message||e);originalLog('REAL_MONEY_LEDGER_PERSIST_LOAD_ERR',lastError);return false}}
for(const [id,symbol,side,qty,avgPrice,leverage,actualRisk,netRR,openedAt] of seed)trades.set(id,{id,status:'UNKNOWN',symbol,side,qty,avgPrice,leverage,actualRisk,netRR,openedAt,source:'CONFIRMED_EXECUTION_SEED'});
const originalLog=console.log.bind(console);if(load()===false)save();
...
function parseBan(s){const m=String(s||'').match(/banned until\s+(\d{10,})/i);return m?Number(m[1]):Date.now()+60000}
async function signed(path,params={}){if(Date.now()<cooldownUntil)throw Error('BINANCE_RATE_LIMIT_COOLDOWN');const p={...params,recvWindow:5000,timestamp:Date.now()},base=qs(p);requestCount++;const r=await fetch(`${REST}${path}?${base}&signature=${encodeURIComponent(sign(base))}`,{headers:{'X-MBX-APIKEY':API_KEY}}),text=await r.text();let j;try{j=JSON.parse(text)}catch{j=text}if(!r.ok){if(r.status===418||Number(j?.code)===-1003)cooldownUntil=Math.max(cooldownUntil,parseBan(j?.msg||text));throw Error(`BINANCE_${r.status}_${j?.code||''}_${j?.msg||text}`)}return j}
function cid(id){return('mh_'+crypto.createHash('sha256').update(String(id)).digest('hex')).slice(0,36)}
function isEntryClientId(x){const s=String(x||'');return s.startsWith('mh_')&&!s.startsWith('mhsl_')&&!s.startsWith('mhtp_')&&!s.startsWith('mhe_')}
function posSide(p){if(String(p.positionSide||'BOTH')==='LONG')return'BUY';if(String(p.positionSide||'BOTH')==='SHORT')return'SELL';return Number(p.positionAmt||0)>=0?'BUY':'SELL'}
