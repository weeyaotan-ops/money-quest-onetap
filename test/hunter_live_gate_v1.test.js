...
assert.equal(bad.challenger.observationalOnly,true);
assert.equal(bad.challenger.liveExecutionChanged,false);
assert.ok(['PASS','WATCH','REJECT'].includes(bad.challenger.verdict));
assert.ok(report.forward.challengerV2);
assert.equal(report.forward.challengerV2.liveExecutionChanged,false);
console.log('hunter_live_gate_v1.test.js PASS',JSON.stringify({nullRegression:true,bad:bad.verdict,badV2:bad.challenger.verdict,good:good.verdict,goodV2:good.challenger.verdict,forward:report.forward}));
