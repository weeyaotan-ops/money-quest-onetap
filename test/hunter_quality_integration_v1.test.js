...
async function run(){
  ...
  assert.equal(a.signalAt,new Date(now).toISOString());assert.equal(a.entry,106);assert.equal(a.selectionVersion,'LEGACY');
  ...
  assert.ok(!fs.readFileSync(path.join(root,'opportunity_hunter_server_v1.js'),'utf8').includes('confirmedSetups('));
